...
--2026-04-22 17:31:32--  https://artifacts.picoctf.net/c/420/readmycert.csr
Resolving artifacts.picoctf.net (artifacts.picoctf.net)... 13.226.251.112, 13.226.251.119, 13.226.251.11, ...
Connecting to artifacts.picoctf.net (artifacts.picoctf.net)|13.226.251.112|:443... connected.
...
2026-04-22 17:31:33 (1.64 MB/s) - ‘readmycert.csr’ saved [997/997]
...
```
#### Notas
se enontraba dentro, era cuestión de consultar el cert 
- **`openssl req`**: Llama a la herramienta de OpenSSL específica para gestionar solicitudes de certificados (PKCS#10).
- **`-in readmycert.csr`**: Especifica el archivo de entrada que quieres examinar (en este caso, `readmycert.csr`).
- **`-noout`**: Evita que el comando imprima la versión codificada de la solicitud (el bloque de texto que empieza con `-----BEGIN CERTIFICATE REQUEST-----`).
- **`-text`**: Imprime el contenido de la solicitud en formato de texto claro, mostrando detalles como el Nombre Distintivo (DN), la clave pública y cualquier atributo adicional.
#### Referencias
